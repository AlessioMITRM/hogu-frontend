    ...
    >
      ...
      <div className="relative w-full rounded-2xl lg:rounded-[2rem] overflow-hidden aspect-[16/10] lg:aspect-[2/1] shadow-md transition-[transform,box-shadow] duration-500 group-hover:shadow-2xl group-hover:-translate-y-2 backface-hidden transform-gpu will-change-transform">
        ...
        <div className="absolute bottom-0 left-0 p-3 lg:p-5 text-white w-full">
          <span className="inline-block bg-white/95 backdrop-blur-md px-1.5 py-0.5 rounded-md text-[9px] lg:text-xs font-bold text-[#4A8A75] uppercase tracking-wider mb-1 shadow-sm">
            {regionKey ? t(regionKey) : region}
          </span>
          <h3 className="text-base lg:text-2xl font-extrabold leading-tight mb-0.5 lg:mb-1">{cityKey ? t(cityKey) : city}</h3>
          <div className="flex items-center gap-1 text-[10px] lg:text-sm text-gray-300 group-hover:text-white transition-colors">
...
export const PopularDestinations = ({
  ...
  return (
    <section className={`mt-8 lg:mt-8 mb-6 lg:mb-12 ${className}`}>
      <h2 className="text-base lg:text-2xl font-bold text-slate-800 mb-3 lg:mb-6">{displayTitle}</h2>
